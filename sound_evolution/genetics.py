...
class Population(object):
    ...
    def next_generation(self, frac_mutate, half_frac_breed):
        ...
        for i in range(len(breeders)-1):
            x = breeders[i].ficken(breeders[i+1])
            self.append_individual(x)

    def find_fittest(self, no_fit):
        """ Find a collection of the fittest individuals of size equal to or
            more than no_fit.
    ...
    def natural_selection(self, no_surviving=None):
        """ Filter out the least fit individuals in the population -
            ...
            <no_surviving> optional argument denoting desired
            no_ of surviving individuals.
