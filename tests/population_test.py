...
def setUp():
    ...
    Mutate_Population = se.genetics.Population(
        4, se.instrument.Instrument, {'const_prob': 0.7, 'max_children': 3})
    Ficken_Population = se.genetics.Population(
        0, se.instrument.Instrument, {'const_prob': 0.7, 'max_children': 3})
    Larger_Population = se.genetics.Population(
        10, se.instrument.Instrument, {'const_prob': 0.7, 'max_children': 3})
    tone_json = open(
...
def test_one_breeding():
    """ A population of size two should have size three after breeding 50
        percent of them.
...
def test_population():
    """ Should create a Population object containing a list of instruments
        with length == size.
